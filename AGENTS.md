...
2. **`storage.objects` RLS proof test** — in progress. The highest-frequency historical bug class (ANTI_PATTERNS #1, #11, #12 — four incidents already), and the one gap `PRACTICES.md` §4 already flags as open. New test file, doesn't touch anything else in flight.
3. **Event-date timeline ordering** — in progress. `getDocuments` currently orders by `captured_at` (upload time), not `document_date` — a bulk-loaded family's documents would render scrambled.
4. Bulk capture (multi-file select) — not started.
5. Load the founder's own family's real documents — doubles as the real eval set (see below). Not started; depends on nothing above except capture being pleasant enough to actually do (#4).
6. Fill `expected.json` for 10–15 of those documents, run `pnpm eval` live on the new dev-tier model — not started.
7. Search (server action over the existing `search_tsv`/`pg_trgm` infra) — not started.
