...
**Resolved 2026-09-08:** `__tests__/rls/storage-objects-rls-proof.test.ts` closes this gap for the `documents` bucket. Also caught a second instance of the same underlying pattern while doing it: the bucket had INSERT/SELECT policies but no DELETE policy at all, so by RLS default-deny nobody — not just other families — could delete an uploaded object, silently. Fixed in `supabase/migrations/20260908000001_storage_documents_rls_delete.sql` (Rule 3 permits user-initiated deletion). UPDATE is deliberately still ungranted — see that migration's comment for why overwrite-in-place is a worse failure mode than "can't delete yet," not an oversight.

**Follow-up (2026-09-08):** running the new suite surfaced a third instance of "looks fine, is silently pointed at the wrong thing" one layer up — a bare `pnpm test` with no env exported fell through to `.env.local`, which holds this project's *real production* Supabase credentials, and ran the RLS proof tests (which create/delete real users) against production. `afterAll` cleanup happened to run and left nothing behind that time, but a crash before cleanup would not have been. Fixed by `lib/test/supabase-test-env.ts`: a dedicated `.env.test.local` (never `.env.local`) plus a hard refusal to proceed if the resolved URL isn't a local address, checked before any Supabase client is even created.
...
**Rule:** when wrapping a third-party call in a try/catch for UX purposes, distinguish *why* it failed before deciding how to fail: a real domain rejection (wrong password, actual rate-limit hit) fails closed with a specific message; an infra/network failure on the call itself should either fail **open** (if the check is a soft guardrail, not a security boundary — e.g. rate limiting) or fail closed with an honest, distinct message ("could not reach the server," never the domain-specific wording) so a real outage is never mistaken for a user-caused problem. Supabase's `isAuthRetryableFetchError()` (from `@supabase/supabase-js`) is the concrete tool for the auth case; the rate-limiter fix (`app/api/uploads/sign/route.ts` — a rejection from `uploadRatelimit.limit()` fails closed with 429, an exception calling it fails open and logs, tested in `__tests__/unit/uploads-sign-ratelimit.test.ts`) is the general pattern for any abuse-prevention check.

## 14. A free-tier third-party dependency going idle looks exactly like a code bug

**Failure mode:** the day after #13's fail-open fix shipped, a production stress test of the rate limiter showed every request succeeding when several should have 429'd. The fail-open log line (`rate limiter unreachable, failing open`, `error: 'fetch failed'`) pointed at Upstash — but Upstash's env vars were present and correctly named in Vercel, so the first instinct was to suspect a credentials or network-egress bug in the new code. The actual cause was outside the codebase entirely: Upstash's free tier deletes a database after 14 days with no REST API traffic, and this project's Redis had gone idle long enough to be auto-deleted. Nothing in the code was wrong; the infra it depended on had silently ceased to exist. This had been invisible before #13's fix only because rate limiting was hard-disabled at the time — the fix didn't cause this, it's what finally surfaced a gap that had existed the whole time.

**Rule:** when a third-party integration starts failing with a connection-level error (`fetch failed`, DNS failure, connection refused) rather than an auth or validation error, check whether the *resource itself* still exists before debugging the calling code — especially for anything provisioned on a free tier, which commonly carries an idle-deletion policy that isn't advertised anywhere in the app's own code or docs. `docs/DECISIONS.md` D-015 is the concrete fix for this specific dependency (a scheduled keepalive ping, `app/api/cron/keepalive`); the general lesson is to ask "does this still exist" before "why is this erroring" whenever a free-tier external service is involved.

**Enforcement:** `app/api/cron/keepalive` + `vercel.json`'s Sunday/Wednesday cron schedule prevents recurrence for Upstash specifically, and a healthchecks.io dead-man's-switch (`HEALTHCHECKS_PING_URL`) alerts by email if the cron itself ever stops running or starts failing (D-015's addendum — Vercel's own Alerts product needs a Pro plan this team doesn't have). No general automated check exists for other free-tier dependencies (Supabase project pausing has a similar idle policy on some plans — worth the same question if it ever errors the same way). The full reusable pattern, written to be copied into other projects, is `docs/PLAYBOOK_FREE_TIER_KEEPALIVE_MONITORING.md`.

## 15. A migration committed to git is not a migration deployed — and a swallowed query error hides it

**Failure mode:** three migrations from a single session (`documents.patient_name_as_written`, the storage-bucket DELETE policy, `documents.event_date`) were written, committed, applied to the local Docker instance, and treated as "done" — but never pushed to the actual Supabase project `.env.local` points at (`npx supabase db push` was never run). Every real capture from that point silently failed to organize (`organizeDocument`'s `.update()` hit "Could not find the 'patient_name_as_written' column of 'documents' in the schema cache"), the storage DELETE policy the delete-document feature depends on was never live, and the timeline was empty for every profile in production. None of this was visible because a second, independent bug compounded it: `lib/dal/timeline.ts`'s `getTimelinePage` did `const { data } = await query`, discarding `error` entirely — a query that fails outright (missing column) renders identically to a profile with zero documents. Two silent failures stacked: a deploy gap that looked like a feature working normally, hidden further by a DAL that had no way to say "this broke" versus "there's nothing here." Found only by dogfooding real documents through the actual app against the real environment — no unit test, Storybook story, or `pnpm lint:arch` run catches an environment that's missing a migration, since they never touch the deployed database at all.

**Rule:** a migration file existing in `supabase/migrations/` and passing `pnpm lint:arch`/tests proves the *code* is correct, never that the *environment currently running the app* has it. Before calling a phase's database work done, run `npx supabase migration list` and confirm the `Local` and `Remote` columns match — a migration present only in `Local` is exactly this bug waiting to happen. Separately and unconditionally: every Supabase query result in a DAL function (`lib/dal/**`) must inspect `error`, not just destructure `data` and fall back to `null`/`[]` — silently swallowing a query error is Anti-Pattern #1's "silent 0-row write" one layer up, at the read side instead of the write side.

**Enforcement:** none automated yet. `npx supabase migration list` is a manual step — worth adding to `docs/PRACTICES.md` §8's phase-exit checklist explicitly (not just implied by "dogfooded"), since "dogfooded locally against Docker" and "dogfooded against the environment `.env.local` actually points at" are not the same claim and this incident is proof the gap between them is real. No lint rule currently catches a DAL function that discards `error` from a Supabase query — same category of gap as #1's original enforcement note.
