...
- **Eval set:** 10–15 real family documents (founder-supplied, anonymised), each with expected extractions (`doc_type`, dates, names, medications-as-written) — grows with every organize failure found in dogfooding (failure → new eval case, permanently). One directory per case under `eval/cases/`: the document, an `expected.json`, and a recorded `cached-response.json`. **Fixture status (synthetic placeholders vs. real documents) lives in `eval/cases/README.md`** — that file is the single source of truth for it.
