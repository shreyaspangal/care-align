...
| 6 | Real, anonymised user documents used as eval fixtures must not become permanent, public data if a redaction is missed | This repository is public (`gh repo view` confirms); no specific authority mandates this, but DPDP's general data-protection posture makes an unreviewed public commit of real health documents an obvious exposure | `.gitignore`'s `eval/cases/real-*/` exclusion (D-016) — real case directories are never committed, by naming convention alone | **Resolved (D-016) — real eval documents can now be added locally without exposure risk. The 10–15 real documents themselves still don't exist yet** |
